...
"""
CALC Configuration Module

This module manages configuration settings for the CALC application.
CALC stores its settings and data in the `FINCH_PATH` directory, which
defaults to `~/.calc` but can be customized using the `FINCH_PATH`
...
Use this module to easily manage and retrieve CALC-specific settings.
"""
...
default = {
    "data_path": str(Path(sysconfig.get_path("data")) / "calc"),
    "cache_size": 10_000,
    "cache_enable": True,
...
config = donfig.Config("calc", defaults=[default])


def get_version():
    """
    Get the version of CALC.
    """
